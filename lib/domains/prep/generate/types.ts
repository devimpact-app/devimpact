...
export type PrepSignalForLLM = {
  ...
  window: {
    startIso: string;
    endIso: string;
    label?: string; // optional: "Last completed week"
  };
  text: string;
  rule?: string; // e.g. "reviewRounds > 1", "timeToFirstReview > 18h"
  primary?: {
    entityType: 'pull_request' | 'review' | 'meeting';
    id: string;
    title: string;
    subtitle?: string; // repo • #123
    url?: string;
    occurredAtIso?: string; // e.g. mergedAt or createdAt
  };
  evidence?: Array<{
    label: string; // "PR", "First review", "Size", "Idle gap"
    value: string; // "18.5h", "1200 lines / 32 files", "3.2 days"
  }>;
};
