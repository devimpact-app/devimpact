...
import {
  MetricWindowKind,
  PrepMetricForLLM,
  PrepMetricStatForLLM,
  PrepMetricTimeseriesPointForLLM,
  PrepMetricWindowForLLM,
} from '../types';

function buildStatForLLM(
  result: TStatResult
): PrepMetricStatForLLM | undefined {
...
): PrepMetricTimeseriesPointForLLM[] | undefined {
...
export function buildOneOnOneMetricsFromBatch(
  ...
  const output: PrepMetricForLLM[] = [];

  for (const [metricId, group] of Object.entries(byMetric)) {
    const windowsForMetric: PrepMetricWindowForLLM[] = [];
