...
function getSizeBucket(
  ...
  if (lines <= 800 && files <= 14) return 'small';
...
export function generateFastLoopsInsight(
  ...
  const baselineMedianHours = computeMedianClamped(
    ...
    { min: 0, max: MAX_HOURS_CUTOFF } // 14 days in hours
  );
  ...
  for (const segmentDef of segmentsToEvaluate) {
    ...
    let baselineMedianHoursForSegment = baselineMedianHours;
    const others = candidates.filter((c) => !segmentDef.matches(c));
    if (others.length >= 2) {
      const otherBaselineMedian = computeMedianClamped(
        others.map((c) => c.cycleTimeHours),
        { min: 0, max: MAX_HOURS_CUTOFF }
      );
      baselineMedianHoursForSegment = otherBaselineMedian;
    }
    ...
    const improvementRatio =
      baselineMedianHoursForSegment > 0
        ? baselineMedianHoursForSegment / fastMedianHours
        : 1;

    // require at least ~30–40% faster and maybe >= 2h absolute difference
    const absoluteDiff = baselineMedianHoursForSegment - fastMedianHours;
    ...
    viableSegments.push({
      ...
      fastMedianHours,
      baselineMedianHours: baselineMedianHoursForSegment,
      improvementRatio,
    });
  }
  ...
  const pctImprovement =
    best.baselineMedianHours > 0
      ? (best.baselineMedianHours - best.fastMedianHours) /
        best.baselineMedianHours
      : 0;

  const body = formatFastLoopsBody({
    segmentLabel: best.label,
    descriptionHint: best.descriptionHint,
    baselineMedianHours: best.baselineMedianHours,
