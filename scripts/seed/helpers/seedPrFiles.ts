...
export type FilesConfig = {
  fileCount?: number; // e.g. 2, 5, 12 (if omitted, use old rand(3,8))
  fileCountRange?: [number, number]; // e.g. [2,4], [5,10] (takes precedence over fileCount)
  totalAddsRange?: [number, number]; // e.g. [40, 120]
  totalDelsRange?: [number, number]; // e.g. [5, 80]
  testBias?: number; // 0–1, how often we mark .ts/.tsx as tests
};
...
export async function seedPrFiles(params: {
  ...
  const { pr, tenantId, authorGithubLogin, filesConfig } = params;
  const defaultBranch = params.repoDefaultBranch ?? 'main';

  // ---- file count ----
  let fileCount: number;
  if (filesConfig?.fileCountRange) {
    const [min, max] = filesConfig.fileCountRange;
    fileCount = rand(min, max);
  } else if (typeof filesConfig?.fileCount === 'number') {
    fileCount = Math.max(1, filesConfig.fileCount);
  } else {
    // old behavior
    fileCount = rand(3, 8);
  }
  ...
  // ---- statuses (same distribution for now) ----
  ...
  // allow archetype to bias tests
  const testBias =
    typeof filesConfig?.testBias === 'number' ? filesConfig.testBias : 0.25; // old default

  for (const status of statuses) {
    const ext = pick(exts);
    const { path, isTest } = makeFilename(pr.repoFullName, ext, testBias);
  ...
  // ---- distribute totals (adds / dels) ----
  const totalAddsRange = filesConfig?.totalAddsRange ?? [30, 300];
  const totalDelsRange = filesConfig?.totalDelsRange ?? [
    5,
    Math.floor(totalAddsRange[1] * 0.6),
  ];

  const totalAdds = rand(totalAddsRange[0], totalAddsRange[1]);
  const totalDels = rand(totalDelsRange[0], totalDelsRange[1]);

  const adds = splitTotals(totalAdds, files.length);
  const dels = splitTotals(totalDels, files.length);

  // ---- upserts ----
  let actualAddsTotal = 0;
  let actualDelsTotal = 0;
  ...
  return {
    count: files.length,
    totalAdds: actualAddsTotal,
    totalDels: actualDelsTotal,
  };
}
